...
function getAreaListcallback(r){
	...
	if (currentAreaInfo.currentLevel >= 2){
		...
		if(page_load){ //初始化加载
			...
			if(currentAreaInfo.currentAreaId && parseInt(currentAreaInfo.currentAreaId)>0){
...
function chooseProvince(provinceId){
	...
	currentAreaInfo.currentProvinceName = getProvinceNameById(provinceId);
	// alert(page_load)
	...
	if(provinceCityJson[provinceId.toString()]){

		cityContainer.html(getAreaList(provinceCityJson[provinceId.toString()]));
		...
		if(page_load){ //初始化加载
			...
			if(currentAreaInfo.currentCityId && parseInt(currentAreaInfo.currentCityId)>0){
...
$("#store-selector .close").css('display', 'block');
...
function CurrentAreaInfoInit(){
	...
	if(ipLoc.length>0&&ipLoc[0]){
		...
		currentAreaInfo.currentProvinceName = getProvinceNameById(ipLoc[0]);
...
// 北京 上海  天津  重庆 区域最后一个添加稍后再说,并改变颜色为红色
function changeColor(){
	dataValue = $('#stock_city_item > .area-list > li:last > a').attr('data-value')
		
	if (dataValue == "1001"){
		$('#stock_city_item > .area-list > li:last > a').css('color', 'red');	
	}
}
